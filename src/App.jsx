...
const WordMemorizerApp = () => {
    ...
    const loadWords = () => {
        try {
        // TODO: This block should be set as
        // get todayWordList, no default list.
        // 
    ...
    // Testing customed column with excel parser first.
    ...
    if (screen === 'session') {
        // session depend on checkAnswer and options settings.
    ...
    if (screen === 'wordlist') {
        // Get all possible keys from the first word, except 'id' and 'emoji'
        const tableHeaders = words.length > 0
            ? Object.keys(words[0]).filter(k => k !== 'id' && k !== 'emoji')
            : [];
            ...
            <div className="max-w-6xl mx-auto">
                ...
                <div className="overflow-x-auto">
                    ...
                    <thead className="bg-white/10">
                        ...
                        <th className="px-4 py-2 text-left border-b border-white/20">Emoji</th>
                        <th className="px-4 py-2 text-left border-b border-white/20">Accuracy</th>
                        </tr>
                    </thead>
                    <tbody>
                        {words.map(word => {
                        ...
                        return (
                            ...
                            <td className="px-4 py-2 border-b border-white/20">{word.emoji || ''}</td>
                            <td className="px-4 py-2 border-b border-white/20">
                                {wordStats.attempts > 0 ? `${accuracy}%` : ''}
                            </td>
                            </tr>
                        );
                        })}
